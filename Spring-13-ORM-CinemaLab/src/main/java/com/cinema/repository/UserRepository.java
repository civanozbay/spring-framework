...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(value = "SELECT * FROM user_account ua JOIN account_details ad" +
            " ON ua.accout_details_id = ad.id WHERE ad.name ILIKE concat('%',?1,'%')",nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM user_account ua JOIN account_details ad " +
            "ON ad.id = ua.account_details_id WHERE ad.age BETWEEN ?1 AND ?2",nativeQuery = true)
