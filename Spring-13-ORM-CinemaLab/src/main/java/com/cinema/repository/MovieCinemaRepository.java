...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    Integer countAllByCinema(Long cinemaId);
    ...
    List<MovieCinema> listMoviesHigherThanSpecificDate(LocalDate date);
    // ------------------- Native QUERIES ------------------- //

    //Write a native query to count all movie cinemas by cinema id
    @Query(value = "Select count(*) from movie_cinema " +
            "where cinema_id = ?1",nativeQuery = true)
    Integer countMovieCinema();
    //Write a native query that returns all movie cinemas by location name
    @Query(value = "SELECT * from movie_cinema m join cinema c on c.id=m.cinema_id" +
            "join on location l c.location_id=l.id where l.name=?1  ",nativeQuery = true)
    List<MovieCinema> getMovieCinemaByLocationName();
